"""Neural-network building blocks for multimodal MRLOD."""
...
class SimpleLinearAdapter(nn.Module):
    def __init__(
        self,
        input_dim: int = 768,
        output_dim: int = 256,
...
class OmicsAnchoredGlobalFusion(nn.Module):
    def __init__(
        self,
        latent_dim: int,
        dropout: float = 0.1,
        ...
        self.context_norm = nn.LayerNorm(latent_dim * 3)
        self.gate = nn.Sequential(
            nn.Linear(latent_dim * 3, rank),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(rank, 3),
        )
    ...
    def forward(
        ...
        context = self.context_norm(
            torch.cat([omics, image - omics, text - omics], dim=-1)
        )
        weights = torch.softmax(self.gate(context), dim=-1)
        mixture = torch.sum(
            normalized * weights.unsqueeze(-1),
            dim=1,
        )
        residual_strength = torch.sigmoid(self.residual_logit)
        anchored = omics + residual_strength * (mixture - omics)
        global_representation = self.out_norm(self.ffn(anchored))
        entropy = -(
