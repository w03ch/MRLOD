"""Train the multimodal MRLOD experiment and save fused latents."""
...
IMAGE_FEATURE_FILENAME = "TCGA_TITAN_features.pkl"
TEXT_FEATURE_FILENAME = "tcga_reports_embeddings.csv"

LATENT_DIM = 256
DIFFUSION_STEPS = 1000
BATCH_SIZE = 128
LEARNING_RATE = 1e-4
FEATURE_MASK_PROBABILITY = 0.25
EPOCHS = 100
...
def require_cuda() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "MRLOD training requires a CUDA-capable GPU and CUDA-enabled "
            "PyTorch. CPU training is not supported by this release."
        )
...
def train_model(
    ...
    print(
        f"\nTraining MRLOD on {dataset_name}: "
    ...
    for epoch_index in range(EPOCHS):
        ...
        for batch in loader:
            ...
            reconstruction_loss = semantic_reconstruction_loss(
                model.reconstruct_semantic_embeddings(
                    global_representation
                ),
                batch_data,
            )
...
def parse_args(
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=REPOSITORY_ROOT / "artifacts" / "multimodal",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    cohort_database = preprocess_multimodal_data(
        str(data_root / "OMIC"),
        str(data_root / IMAGE_FEATURE_FILENAME),
        str(data_root / TEXT_FEATURE_FILENAME),
