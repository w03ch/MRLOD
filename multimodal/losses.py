"""Losses used by the fixed multimodal MRLOD experiment."""
...
def semantic_reconstruction_loss(
    ...
    return (
        F.mse_loss(reconstructions["image"], targets["image"])
        + F.mse_loss(reconstructions["text"], targets["text"])
    )
