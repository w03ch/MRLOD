...
class MRLOD(nn.Module):
    ...
    SEMANTIC_MODALITIES = ("image", "text")
    MODALITIES = OMICS_MODALITIES + SEMANTIC_MODALITIES
    CLUSTER_VIEWS = ("omics", *SEMANTIC_MODALITIES)
    ...
    def __init__(
        ...
        for modality in self.SEMANTIC_MODALITIES:
            self.encoders[modality] = SimpleLinearAdapter(
                input_dim=input_dims[modality],
                output_dim=latent_dim,
            )
        ...
        self.contribution_fusion = OmicsAnchoredGlobalFusion(
            latent_dim=latent_dim,
            dropout=0.1,
            rank=64,
        )
        ...
        self.image_text_rec_decoders = nn.ModuleDict()
        for modality in self.SEMANTIC_MODALITIES:
            self.image_text_rec_decoders[modality] = nn.Linear(
                latent_dim,
                input_dims[modality],
            )
    ...
    def get_global_and_token_embeddings(
        ...
        for modality in self.SEMANTIC_MODALITIES:
            raw_tokens[modality] = self.encoders[modality](
                batch_data[modality]
            ).unsqueeze(1)
        ...
        global_representation, fusion_statistics = (
            self.contribution_fusion(fusion_sequence)
        )
        ...
        modality_tokens.update(
            {
                "omics": omics_consensus.squeeze(1),
                "omics_weights": omics_statistics["omics_weights"],
                "image": raw_tokens["image"].squeeze(1),
                "text": raw_tokens["text"].squeeze(1),
                "modality_weights": fusion_statistics[
                    "modality_weights"
                ],
                "modality_entropy": fusion_statistics[
                    "modality_entropy"
                ],
            }
        )
    ...
    def reconstruct_semantic_embeddings(
        self,
        global_representation: torch.Tensor,
    ) -> Dict[str, torch.Tensor]:
        return {
            modality: decoder(global_representation)
            for modality, decoder in self.image_text_rec_decoders.items()
        }
